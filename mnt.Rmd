...
## 2. Code de téléchargement du MNT

```{r mnt, eval=FALSE, echo=TRUE}
# Zone d'étude centrée sur la Baie de Morlaix
point <- st_sfc(st_point(c(-3.85, 48.65)), crs = 4326)
zone <- st_buffer(st_transform(point, 2154), 20000)

# Téléchargement du MNT via get_wms_raster
...
## 3. Code de création des points éoliennes

```{r eoliennes, eval=FALSE, echo=TRUE}
# Communes de Roscoff et Plougasnou
...
# Point éolienne au large de la Baie de Morlaix
...
## 4. Code du calcul du Viewshed

```{r viewshed, eval=FALSE, echo=TRUE}
# Calcul du viewshed
...
```{r comparaison, eval=FALSE, echo=TRUE}
